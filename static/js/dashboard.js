$(function () {
    ...
    let scoreChart = document.getElementsByClassName('score-chart');

    for (let i = 0; i < scoreChart.length; i++) {
        let scores = scoreChart[i].getAttribute('data-value');

        let scoreLists = [0, 0, 0, 0];
        if (scores) {
            scoreLists = scores.split(",").map(function (item) {
        ...
        new ApexCharts(scoreChart[i], options).render();
    }

    // Progressgar1 start
    var seasonResult = $('#progressbar1');

    if (seasonResult.length) {
        var range = seasonResult.attr('data-value');
